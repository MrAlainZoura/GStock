...
class VenteController extends Controller
{
     ...
     */
    public function showDepotVente($depot_id, $tranche = null){
    
        $depot= Depot::find($depot_id);//where('libele',$depotVar)->where('id', $id)->first();
        $tranche = ($tranche)? $tranche : 0;
        if($depot){
            $month = Carbon::now()->subMonths($tranche)->format('Y-m-d');
            $vente = $depot->vente()
                    ->with($this->relation)->get()->reverse();
                    // ->where('created_at', '>=',$month )
                    // ->where('created_at', '<=', Carbon::today()->endOfDay());
            return response()->json(['success'=>true, 'data'=>$vente]);

          }else{
            return response()->json(['success'=>false, 'data'=>[], 'error'=>"depot not found"]);
    ...
    {
        $detailVente = Vente::where('id',$id)->with($this->relation)->first();
        if(!$detailVente){
            return response()->json(['success'=>false, 'data'=>[], 'error'=>"vente not found"]);
        }
        
        $depotId = $detailVente->depot->id;
        $userRole = auth('api')->user()->user_role->role->libele;
        $user_id = auth('api')->user()->id;
        $userDepotAffectation = auth('api')->user()->depotUser;
        // return response()->json([$user_id, $userRole, $detailVente]);
        ...
        if (!in_array($userRole, $roleAutorises)) {
            if($existAffectation === null){
                return response()->json(['success'=>false, 'data'=>[], 'error'=>"Vous n'avez pas accès à cette information"]);
            }
            return response()->json(['success'=>true, 'data'=>$detailVente]);
        }
        if($userRole === $roleAutorises[0]){
                $adminDepotCreateur = auth('api')->user()->depot->filter(function ($u) use ($user_id) {
