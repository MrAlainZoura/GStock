...
<section style="background-image: url('{{ asset('img/depot.jpg') }}');"  class="bg-center bg-no-repeat bg-cover bg-gray-700 bg-blend-multiply">
    <div class="px-4 mx-auto max-w-screen-xl text-center py-24 lg:py-56">
        ...
        <div class="flex flex-col space-y-4 sm:flex-row sm:justify-center sm:space-y-0">
            <a href="#sectionDepot" class="inline-flex justify-center items-center py-3 px-5 text-base font-medium text-center text-white rounded-lg bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 dark:focus:ring-blue-900">
                Commencons 
                {{$user->name}}
            ...
            @if ($user->user_role->role->libele=='Super admin')
                <a href="{{ route("admin.index") }}" class="inline-flex justify-center hover:text-gray-900 items-center py-3 px-5 sm:ms-4 text-base font-medium text-center text-white rounded-lg border border-white hover:bg-gray-100 focus:ring-4 focus:ring-gray-400">
                    A vous de jouer
                </a>  
            @elseif($user->user_role->role->libele=='Administrateur')
                <a href="{{route('abonnement.list', $user->id*13)}}" class="inline-flex justify-center hover:text-gray-900 items-center py-3 px-5 sm:ms-4 text-base font-medium text-center text-white rounded-lg border border-white hover:bg-gray-100 focus:ring-4 focus:ring-gray-400">
                    A vous de jouer
                </a> 
            @else
             <a href="#sectionDepot" class="inline-flex justify-center hover:text-gray-900 items-center py-3 px-5 sm:ms-4 text-base font-medium text-center text-white rounded-lg border border-white hover:bg-gray-100 focus:ring-4 focus:ring-gray-400">
                    A vous de jouer
                </a>
            @endif
        </div>
