<div class="grid grid-cols-1 gap-2 md:grid-cols-3">
    ...
    @elseif(Auth::user()->user_role->role->libele =='Administrateur' || Auth::user()->user_role->role->libele=='Super admin')
        ...
        @endif
        @if (Route::current()->getName()=="venteTrashed" )
            <a title="Restorer cette vente" itemName ="{{$itemName}}" 
            href="{{route("admin.confirmDeleteItem", 
            ["id"=>$editParam,
             "message"=>$itemName,
             "route"=>Crypt::encrypt($editeRoute)])}}">
        ...
        @elseif(Route::current()->getName()=="aproDepot")
            <form action="{{route($editeRoute, $editParam)}}" method="post">
                <!-- confirmation de l'approvisionnement -->
                @csrf
                @method('post')
                <button type="submit" title="Confirmer approvisionnement" itemName ="{{$itemName}}">
            ...
            <a title="Supprimer" role="button" itemName ="{{$itemName}}" id="linkDelete0" href="{{route("admin.confirmDeleteItem", 
            ["id"=>$deleteParam,
             "message"=>$itemName,
             "route"=>Crypt::encrypt($deleteRoute)])}}">
                <svg class="w-[26px] h-[26px] text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
